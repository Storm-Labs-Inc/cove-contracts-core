...
contract RebalancingUtils {
    ...
    struct StrategyData {
        ...
        address rebalancingUtilsAddress;
        /// STATE VARIABLES ///
        ...
        mapping(address basketToken => mapping(address basketAsset => uint256 indexPlusOne)) _basketAssetToIndexPlusOne;
        /// @notice Mapping of basket token to index plus one. 0 means the basket token does not exist.
        mapping(address basketToken => uint256 indexPlusOne) _basketTokenToIndexPlusOne;
        ...
        bytes32 _externalTradesHash;
    }

    bytes32 internal constant BASE_STRATEGY_STORAGE = bytes32(uint256(keccak256("cove.basket.manager.storage")) - 1);
    ...
    {
        // Checks
        if (basketTokenImplementation_ == address(0)) revert Errors.ZeroAddress();
        ...
        require(S.basketTokenImplementation == address(0), "initialized");
        S.basketTokenImplementation = basketTokenImplementation_;
        S.eulerRouter = EulerRouter(eulerRouter_);
        S.strategyRegistry = StrategyRegistry(strategyRegistry_);
        S.rebalancingUtilsAddress = rebalancingUtilsAddress_;
    }
        ...
        returns (address basket)
    {
        ...
        uint256 basketTokensLength = S.basketTokens.length;
        ...
        if (S.basketIdToAddress[basketId] != address(0)) {
            revert BasketTokenAlreadyExists();
        }
        // Checks with external view calls
        if (!S.strategyRegistry.supportsBitFlag(bitFlag, strategy)) {
        ...
        address[] memory assets = S.strategyRegistry.getAssets(bitFlag);
        if (assets.length == 0) {
        ...
        // Effects
        basket = Clones.clone(S.basketTokenImplementation);
        S.basketTokens.push(basket);
        S.basketAssets[basket] = assets;
        uint256 assetsLength = assets.length;
        for (uint256 j = 0; j < assetsLength;) {
            // nosemgrep: solidity.performance.state-variable-read-in-a-loop.state-variable-read-in-a-loop
            S._basketAssetToIndexPlusOne[basket][assets[j]] = j + 1;
            unchecked {
                ++j;
            }
        }
        // Interactions
        S.basketIdToAddress[basketId] = basket;
        unchecked {
            // Overflow not possible: basketTokensLength is less than the constant _MAX_NUM_OF_BASKET_TOKENS
            S._basketTokenToIndexPlusOne[basket] = basketTokensLength + 1;
        }
        // Interactions
    ...
    function basketTokenToIndex(address basketToken) external view returns (uint256 index) {
        StrategyData storage S = _strategyStorage();
        index = S._basketTokenToIndexPlusOne[basketToken];
        if (index == 0) {
    ...
    {
        StrategyData storage S = _strategyStorage();
        index = S._basketAssetToIndexPlusOne[basketToken][asset];
        if (index == 0) {
    ...
    function externalTradesHash() external view returns (bytes32) {
        StrategyData storage S = _strategyStorage();
        return S._externalTradesHash;
    }

    function eulerRouter() external view returns (EulerRouter) {
        StrategyData storage S = _strategyStorage();
        return S.eulerRouter;
    }

    function basketTokens() external view returns (address[] memory) {
        StrategyData storage S = _strategyStorage();
        return S.basketTokens;
    }

    function basketIdToAddress(bytes32 basketId) external view returns (address) {
        StrategyData storage S = _strategyStorage();
        return S.basketIdToAddress[basketId];
    }

    function basketBalanceOf(address basketToken, address asset) external view returns (uint256) {
        StrategyData storage S = _strategyStorage();
        return S.basketBalanceOf[basketToken][asset];
    }

    function basketTokenImplementation() external view returns (address) {
        StrategyData storage S = _strategyStorage();
        return S.basketTokenImplementation;
    }

    function strategyRegistry() external view returns (StrategyRegistry) {
        StrategyData storage S = _strategyStorage();
    ...
    function proposeRebalance(address[] calldata basketsToRebalance) external {
        ...
        if (S.rebalanceStatus.status != Status.NOT_STARTED) {
        ...
        for (uint256 i = 0; i < length;) {
            ...
            // nosemgrep: solidity.performance.state-variable-read-in-a-loop.state-variable-read-in-a-loop
            address[] memory assets = S.basketAssets[basket];
            ...
            for (uint256 j = 0; j < assetsLength;) {
                // nosemgrep: solidity.performance.state-variable-read-in-a-loop.state-variable-read-in-a-loop
                balances[j] = S.basketBalanceOf[basket][assets[j]];

                // Rounding direction: down
                // nosemgrep: solidity.performance.state-variable-read-in-a-loop.state-variable-read-in-a-loop
                basketValue += S.eulerRouter.getQuote(balances[j], assets[j], _USD_ISO_4217_CODE);
                unchecked {
            ...
            uint256 pendingRedeems_ = BasketToken(basket).prepareForRebalance();
            {
                ...
                if (pendingDeposit > 0) {
                    ...
                    // nosemgrep: solidity.performance.state-variable-read-in-a-loop.state-variable-read-in-a-loop
                    uint256 pendingDepositValue = S.eulerRouter.getQuote(pendingDeposit, assets[0], _USD_ISO_4217_CODE);
                    // Rounding direction: down
                    ...
                    // nosemgrep: solidity.performance.state-variable-read-in-a-loop.state-variable-read-in-a-loop
                    S.basketBalanceOf[basket][assets[0]] = balances[0] = balances[0] + pendingDeposit;
            ...
            uint256[] memory proposedTargetWeights = BasketToken(basket).getTargetWeights();
            {
                ...
                if (pendingRedeems_ > 0) {
                    ...
                    // nosemgrep: solidity.performance.state-variable-read-in-a-loop.state-variable-read-in-a-loop
                    S.pendingRedeems[basket] = pendingRedeems_;
                }
                ...
                // nosemgrep: solidity.performance.state-variable-read-in-a-loop.state-variable-read-in-a-loop
                targetBalances[0] += S.eulerRouter.getQuote(requiredWithdrawValue, _USD_ISO_4217_CODE, assets[0]);
            }
            ...
            for (uint256 j = 0; j < assetsLength;) {
                ...
                if (
                    S.eulerRouter.getQuote(diff, assets[j], _USD_ISO_4217_CODE) > 500 // nosemgrep
                ) {
                    shouldRebalance = true;
                    break;
                }
        ...
        S.rebalanceStatus.basketHash = keccak256(abi.encodePacked(basketsToRebalance));
        S.rebalanceStatus.timestamp = uint40(block.timestamp);
        S.rebalanceStatus.status = Status.REBALANCE_PROPOSED;
    }
    ...
    {
        StrategyData storage S = _strategyStorage();
        RebalanceStatus memory status = S.rebalanceStatus;
        ...
        _initializeBasketData(basketsToRebalance, afterTradeBasketAssetAmounts_, totalBasketValue_, S);
        _settleInternalTrades(internalTrades, basketsToRebalance, afterTradeBasketAssetAmounts_, S);
        _validateExternalTrades(externalTrades, basketsToRebalance, totalBasketValue_, afterTradeBasketAssetAmounts_, S);
        _validateTargetWeights(basketsToRebalance, afterTradeBasketAssetAmounts_, totalBasketValue_, S);

        status.timestamp = uint40(block.timestamp);
        status.status = Status.TOKEN_SWAP_PROPOSED;
        S.rebalanceStatus = status;
        S._externalTradesHash = keccak256(abi.encode(externalTrades));
    }
    ...
    function completeRebalance(address[] calldata basketsToRebalance) external {
        ...
        RebalanceStatus memory rebalanceStatus_ = S.rebalanceStatus;
        // slither-disable-next-line incorrect-equality
        if (rebalanceStatus_.status == Status.NOT_STARTED) {
            revert NoRebalanceInProgress();
        }
        // Check if the given baskets are the same as the ones proposed
        if (keccak256(abi.encodePacked(basketsToRebalance)) != rebalanceStatus_.basketHash) {
            revert BasketsMismatch();
        }
        // Check if the rebalance was proposed more than 15 minutes ago
        // slither-disable-next-line timestamp
        if (block.timestamp - rebalanceStatus_.timestamp < 15 minutes) {
            revert TooEarlyToCompleteRebalance();
        }
        // TODO: Add more checks for completion at different stages

        // Reset the rebalance status
        S.rebalanceStatus.basketHash = bytes32(0);
        S.rebalanceStatus.timestamp = uint40(block.timestamp);
        S.rebalanceStatus.status = Status.NOT_STARTED;
        ...
        for (uint256 i = 0; i < basketsToRebalance.length;) {
            ...
            // nosemgrep: solidity.performance.state-variable-read-in-a-loop.state-variable-read-in-a-loop
            uint256 pendingRedeems_ = S.pendingRedeems[basket];
            if (pendingRedeems_ > 0) {
                // slither-disable-next-line costly-loop
                delete S.pendingRedeems[basket]; // nosemgrep
                // Assume the first asset listed in the basket is the base asset
                // Rounding direction: down
                ...
                // nosemgrep: solidity.performance.state-variable-read-in-a-loop.state-variable-read-in-a-loop
                uint256 withdrawAmount = S.eulerRouter.getQuote(rawAmount, _USD_ISO_4217_CODE, assets[0]);
                // slither-disable-end calls-loop
                if (withdrawAmount <= balances[0]) {
                    unchecked {
                        // Overflow not possible: withdrawAmount is less than or equal to balances[0]
                        // nosemgrep: solidity.performance.state-variable-read-in-a-loop.state-variable-read-in-a-loop
                        S.basketBalanceOf[basket][assets[0]] = balances[0] - withdrawAmount;
                    }
    ...
    function proRataRedeem(uint256 totalSupplyBefore, uint256 burnedShares, address to) public {
        StrategyData storage S = _strategyStorage();
        // Checks
        ...
        address[] storage assets = S.basketAssets[basket];
        uint256 assetsLength = assets.length;
        // Interactions
        for (uint256 i = 0; i < assetsLength;) {
            address asset = assets[i];
            // nosemgrep: solidity.performance.state-variable-read-in-a-loop.state-variable-read-in-a-loop
            uint256 balance = S.basketBalanceOf[basket][asset];
            // Rounding direction: down
            ...
            if (amountToWithdraw > 0) {
                // nosemgrep: solidity.performance.state-variable-read-in-a-loop.state-variable-read-in-a-loop
                S.basketBalanceOf[basket][asset] = balance - amountToWithdraw;
        ...
        view
    {
        ...
        for (uint256 i = 0; i < numBaskets;) {
            ...
            for (uint256 j = 0; j < assetsLength;) {
                ...
                // nosemgrep: solidity.performance.state-variable-read-in-a-loop.state-variable-read-in-a-loop
                uint256 currentAssetAmount = S.basketBalanceOf[basket][asset];
                afterTradeBasketAssetAmounts_[i][j] = currentAssetAmount;
                // nosemgrep: solidity.performance.state-variable-read-in-a-loop.state-variable-read-in-a-loop
                totalBasketValue_[i] += S.eulerRouter.getQuote(currentAssetAmount, asset, _USD_ISO_4217_CODE);
                unchecked {
                    // Overflow not possible: j is less than assetsLength
                    ++j;
                }
        ...
        private
    {
        for (uint256 i = 0; i < internalTrades.length;) {
            ...
            // nosemgrep: solidity.performance.state-variable-read-in-a-loop.state-variable-read-in-a-loop
            info.buyAmount = S.eulerRouter.getQuote(trade.sellAmount, trade.sellToken, trade.buyToken);
            ...
            // nosemgrep: solidity.performance.state-variable-read-in-a-loop.state-variable-read-in-a-loop
            S.basketBalanceOf[trade.fromBasket][trade.sellToken] = afterTradeBasketAssetAmounts_[info.fromBasketIndex][info
                .sellTokenAssetIndex] = S.basketBalanceOf[trade.fromBasket][trade.sellToken] - trade.sellAmount; // nosemgrep
            // nosemgrep: solidity.performance.state-variable-read-in-a-loop.state-variable-read-in-a-loop
            S.basketBalanceOf[trade.fromBasket][trade.buyToken] = afterTradeBasketAssetAmounts_[info.fromBasketIndex][info
                .buyTokenAssetIndex] = S.basketBalanceOf[trade.fromBasket][trade.buyToken] + info.buyAmount; // nosemgrep
            // nosemgrep: solidity.performance.state-variable-read-in-a-loop.state-variable-read-in-a-loop
            S.basketBalanceOf[trade.toBasket][trade.buyToken] = afterTradeBasketAssetAmounts_[info.toBasketIndex][info
                .toBasketBuyTokenIndex] = S.basketBalanceOf[trade.toBasket][trade.buyToken] - info.buyAmount; // nosemgrep
            // nosemgrep: solidity.performance.state-variable-read-in-a-loop.state-variable-read-in-a-loop
            S.basketBalanceOf[trade.toBasket][trade.sellToken] = afterTradeBasketAssetAmounts_[info.toBasketIndex][info
                .toBasketSellTokenIndex] = S.basketBalanceOf[trade.toBasket][trade.sellToken] + trade.sellAmount; // nosemgrep
            unchecked {
                ++i;
            }
        ...
        private
    {
        for (uint256 i = 0; i < externalTrades.length;) {
            ...
            for (uint256 j = 0; j < basketTradeOwnershipLength;) {
                ...
                afterTradeBasketAssetAmounts_[ownershipInfo.basketIndex][ownershipInfo.buyTokenAssetIndex]
                    + ownershipBuyAmount;
                // Update total basket value TODO: check if this is needed
                totalBasketValue_[ownershipInfo.basketIndex] = totalBasketValue_[ownershipInfo.basketIndex]
                // nosemgrep: solidity.performance.state-variable-read-in-a-loop.state-variable-read-in-a-loop
                - S.eulerRouter.getQuote(ownershipSellAmount, trade.sellToken, _USD_ISO_4217_CODE)
                // nosemgrep: solidity.performance.state-variable-read-in-a-loop.state-variable-read-in-a-loop
                + S.eulerRouter.getQuote(ownershipBuyAmount, trade.buyToken, _USD_ISO_4217_CODE);
                unchecked {
                    ++j;
                }
            }
            // nosemgrep: solidity.performance.state-variable-read-in-a-loop.state-variable-read-in-a-loop
            info.sellValue = S.eulerRouter.getQuote(trade.sellAmount, trade.sellToken, _USD_ISO_4217_CODE);
            // nosemgrep: solidity.performance.state-variable-read-in-a-loop.state-variable-read-in-a-loop
            info.internalMinAmount = S.eulerRouter.getQuote(info.sellValue, _USD_ISO_4217_CODE, trade.buyToken);
        ...
        view
    {
        ...
        for (uint256 i = 0; i < basketsToRebalance.length;) {
            ...
            for (uint256 j = 0; j < proposedTargetWeightsLength;) {
                ...
                // nosemgrep: solidity.performance.state-variable-read-in-a-loop.state-variable-read-in-a-loop
                 S.eulerRouter.getQuote(afterTradeBasketAssetAmounts_[i][j], asset, _USD_ISO_4217_CODE);
                // Rounding direction: down
