...
string constant Artifact_RedstoneCoreOracle = "RedstoneCoreOracle.sol:RedstoneCoreOracle";
...
/// @dev If the contract is found in src/ or directly referenced in src/, it won't need a custom deployer function
/// as it will be generated by forge-deploy. However, if the contract is only referenced in a script/ or test/ folder,
/// it may need a custom deployer function to be deployed. Before adding a custom deployer function, run pnpm run build
/// to check
/// if the deployer function will be generated by forge-deploy.
...
library CustomDeployerFunctions {
    ...
    function deploy_RedstoneCoreOracle(
        Deployer deployer,
        string memory name,
        address base,
        address quote,
        ...
        internal
        returns (RedstoneCoreOracle)
    {
        bytes memory args = abi.encode(base, quote, feedId, feedDecimals, maxStaleness);
        return RedstoneCoreOracle(DefaultDeployerFunction.deploy(deployer, name, Artifact_RedstoneCoreOracle, args));
    }
    ...
    {
        bytes memory args = abi.encode(base, quote, feedId, feedDecimals, maxStaleness);
        return RedstoneCoreOracle(
            DefaultDeployerFunction.deploy(deployer, name, Artifact_RedstoneCoreOracle, args, options)
        );
    }
