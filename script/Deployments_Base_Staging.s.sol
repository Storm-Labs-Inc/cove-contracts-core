...
contract DeploymentsBaseStaging is Deployments {
    ...
    function _setPermissionedAddresses() internal virtual override {
        // Base staging deploy
        // Using placeholder addresses for staging environment
        admin = BASE_STAGING_COMMUNITY_MULTISIG;
        treasury = BASE_STAGING_COMMUNITY_MULTISIG;
        pauser = BASE_STAGING_AWS_KEEPER;
        manager = BASE_STAGING_OPS_MULTISIG;

        // Try to get existing timelock or deploy new one for Base Staging
        address existingTimelock = getAddress(buildTimelockControllerName());
        if (existingTimelock == address(0)) {
            // Deploy a timelock for Base Staging
        ...
        rebalanceProposer = BASE_STAGING_AWS_KEEPER;
        tokenSwapProposer = BASE_STAGING_AWS_KEEPER;
        tokenSwapExecutor = BASE_STAGING_AWS_KEEPER;
        rewardToken = address(0); // No COVE token on Base yet
    }

    function _feeCollectorSalt() internal pure override returns (bytes32) {
        return keccak256(abi.encodePacked("Base_Staging_FeeCollector_2025_10"));
    }
    ...
    function _cleanPermissionsExtra() internal override {
        ...
        ManagedWeightStrategy mwStrategy =
            ManagedWeightStrategy(getAddressOrRevert(buildManagedWeightStrategyName("Gauntlet V1 Base Staging")));
        if (shouldBroadcast) {
            vm.startBroadcast();
        }
        if (mwStrategy.hasRole(DEFAULT_ADMIN_ROLE, COVE_DEPLOYER_ADDRESS)) {
            ...
            if (mwStrategy.hasRole(MANAGER_ROLE, COVE_DEPLOYER_ADDRESS)) {
                // Grant MANAGER_ROLE to AWS Keeper before revoking from deployer
                mwStrategy.grantRole(MANAGER_ROLE, BASE_STAGING_AWS_KEEPER);
    ...
    function _deployNonCoreContracts() internal override {
        ...
        // Basket assets for Base Staging - using same assets as production
        ...
        // Initial weights for staging (equal weighting for testing)
        ...
        // Deploy staging strategy
        _deployManagedStrategy(BASE_STAGING_GAUNTLET_SPONSOR, "Gauntlet V1 Base Staging");

        // Set the initial weights for the strategy and deploy basket token
        // Note: BasketToken will prefix symbol with "cove", so "bUSDstg" becomes "covebUSDstg"
        _setInitialWeightsAndDeployBasketToken(
            BasketTokenDeployment({
                name: "bcoveUSD-staging",
                symbol: "bUSDstg",
                rootAsset: BASE_USDC,
                bitFlag: assetsToBitFlag(basketAssets),
                strategy: getAddressOrRevert(buildManagedWeightStrategyName("Gauntlet V1 Base Staging")),
        ...
        // Set sponsor to placeholder
        if (FeeCollector(feeCollector).hasRole(DEFAULT_ADMIN_ROLE, COVE_DEPLOYER_ADDRESS)) {
            if (shouldBroadcast) {
                vm.broadcast();
            }
            FeeCollector(feeCollector).setSponsor(basketToken, BASE_STAGING_GAUNTLET_SPONSOR);
        }
