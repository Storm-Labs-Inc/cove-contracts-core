...
contract ProductionDeployBasicRetryOperator is DeployScript, Constants, StdAssertions, BuildDeploymentJsonNames {
    ...
    function deploy(bool shouldBroadcast_) public {
        ...
        address basicRetryOperator = address(
            deployer.deploy_BasicRetryOperator(buildBasicRetryOperatorName(), COVE_OPS_MULTISIG, COVE_DEPLOYER_ADDRESS)
        );
        address basketToken = deployer.getAddress(buildBasketTokenName("USD"));

        if (shouldBroadcast) vm.broadcast();
        BasicRetryOperator(basicRetryOperator).approveDeposits(BasketToken(basketToken), type(uint256).max);
