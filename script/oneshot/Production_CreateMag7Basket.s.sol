...
contract ProductionCreateMag7Basket is DeployScript, Constants, BatchScript, BuildDeploymentJsonNames {
    using DeployerFunctions for Deployer;
    using CustomDeployerFunctions for Deployer;

    address public ops_safe = COVE_OPS_MULTISIG;
    address public community_safe = COVE_COMMUNITY_MULTISIG;
        ...
        isBatch(community_safe)
    {
        ...
        addToBatch(
            address(feeCollector), 0, abi.encodeCall(feeCollector.setProtocolTreasury, (COVE_COMMUNITY_MULTISIG))
        );
    }
