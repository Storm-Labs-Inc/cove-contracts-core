...
contract VerifyStatesBaseStaging is Script, VerifyStatesCommon {
    ...
    function _verifyBasketManager() internal view {
        ...
        require(address(basketManager) != address(0), "BasketManager not initialized");

        _logRoleCheck(
            "DEFAULT_ADMIN_ROLE",
            basketManager.hasRole(DEFAULT_ADMIN_ROLE, BASE_STAGING_COMMUNITY_MULTISIG),
            BASE_STAGING_COMMUNITY_MULTISIG
        );
        _logRoleCheck(
            "MANAGER_ROLE", basketManager.hasRole(MANAGER_ROLE, BASE_STAGING_OPS_MULTISIG), BASE_STAGING_OPS_MULTISIG
        );
        _logRoleCheck(
            "TIMELOCK_ROLE",
    ...
    function _verifyFeeCollector() internal view {
        ...
        require(
            feeCollector.hasRole(DEFAULT_ADMIN_ROLE, BASE_STAGING_COMMUNITY_MULTISIG),
            "FeeCollector: admin role missing"
        );
        _logRoleCheck(
            "DEFAULT_ADMIN_ROLE",
            feeCollector.hasRole(DEFAULT_ADMIN_ROLE, BASE_STAGING_COMMUNITY_MULTISIG),
    ...
    function _verifyAssetRegistry() internal view {
        ...
        require(address(assetRegistry) != address(0), "AssetRegistry not initialized");
    ...
    function _verifyStrategyRegistry() internal view {
        ...
        address strategyAddr = deployer.getAddress(buildManagedWeightStrategyName("Gauntlet V1 Base Staging"));
        ...
        console.log(
            string.concat(
                ...
                " (",
                buildManagedWeightStrategyName("Gauntlet V1 Base Staging"),
                ")"
            )
        );
    }
    ...
    function _verifyBasicRetryOperator() internal view {
        ...
        require(
            basicRetryOperator.hasRole(DEFAULT_ADMIN_ROLE, BASE_STAGING_COMMUNITY_MULTISIG),
            "BasicRetryOperator: admin role missing"
        );
        _logRoleCheck(
            "DEFAULT_ADMIN_ROLE",
            basicRetryOperator.hasRole(DEFAULT_ADMIN_ROLE, BASE_STAGING_COMMUNITY_MULTISIG),
    ...
    function _verifyManagedWeightStrategy() internal view {
        ...
        _logRoleCheck(
            "DEFAULT_ADMIN_ROLE",
            mws.hasRole(DEFAULT_ADMIN_ROLE, BASE_STAGING_COMMUNITY_MULTISIG),
            BASE_STAGING_COMMUNITY_MULTISIG
        );
        _logRoleCheck("MANAGER_ROLE", mws.hasRole(MANAGER_ROLE, BASE_STAGING_AWS_KEEPER), BASE_STAGING_AWS_KEEPER);
        require(
    ...
    function _verifyBasketToken() internal view {
        ...
        address basketTokenAddr = deployer.getAddress(buildBasketTokenName("bcoveUSD-staging"));
